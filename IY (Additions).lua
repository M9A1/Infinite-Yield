...
local function connectDetachEvents(plr_name, cmd) -- ADDED
	local player = Players:FindFirstChild(plr_name)
	local enemy = player.Character:FindFirstChildWhichIsA("Humanoid")
...
addcmd("bang", {"rape"}, function(args, speaker)
	...
	if args[1] then
		...
		-- ADDED
		if players then
			connectDetachEvents(players[1], "unbang")
		end
		-- END ADDED
...
addcmd('carpet',{},function(args, speaker)
	if not r15(speaker) then
		...
		-- ADDED
		if players then
			connectDetachEvents(players[1], "uncarpet")
		end
		-- END ADDED
